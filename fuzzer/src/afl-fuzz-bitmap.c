...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  if (likely(afl->haste_mode)) {
    
    u64 cksum_64 = 0;

    if (likely(cksum_flag == 0)) {
      
      cksum_64 = hash64(afl->fsrv.trace_bits, afl->fsrv.map_size, HASH_CONST);
    ...
    u32 cksum_32 = 0;
    cksum_32 = ((cksum_64 >> 34) ^ (cksum_64 & 0x3FFFFFFF)) & 0x3FFFFFFF; // shorten the 64-bit hash value to 31-bit

    cksum_32 += 1;

    //if (cksum_32 > MAX_ALLOC) FATAL("cksum for record bitmap in hastemode is too large!!");
    /* check whether the execution is interesing (the trace is unique) */
    if (likely((afl->hastemode_records_map)[cksum_32-1] == 1)) return 0;

    if (afl->haste_mode == 2) {

      /* reserve original bitmap */
      u8* tmp_trace_bits = ck_alloc_nozero(afl->fsrv.map_size);
      memcpy(tmp_trace_bits, afl->fsrv.trace_bits, afl->fsrv.map_size);
      ...
      /* restore */
      memcpy(afl->fsrv.trace_bits, tmp_trace_bits, afl->fsrv.map_size);
      ck_free(tmp_trace_bits);

    }

    (afl->hastemode_records_map)[cksum_32-1] = 1;
