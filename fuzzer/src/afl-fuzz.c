...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Ab:B:c:CdDe:E:hi:I:f:F:g:G:l:L:m:M:nNOo:p:RQs:S:t:T:UV:WXx:YZu:")) >
      0) {
    ...
    switch (opt) {
      ...
      case 'u':
        afl->haste_mode = 2;
        afl->hastemode_binary = ck_strdup(optarg);
        break;
  ...
  if (afl->haste_mode &&
      (!strcmp("-", afl->hastemode_binary) || !strcmp("0", afl->hastemode_binary))) {

    afl->haste_mode = 1;
    afl->hastemode_binary = strdup(argv[optind]);
  ...
  if (afl->haste_mode) {
    ...
    ACTF("Spawning hastemode forkserver");
    afl_fsrv_init_dup(&afl->hastemode_fsrv, &afl->fsrv);

    /* haste mode binary dose not need the trace bits, here we just use the original shared memory,
     maybe optimizing in instrumentation side later */
    afl->hastemode_fsrv.trace_bits = afl->fsrv.trace_bits;
    afl->hastemode_fsrv.cs_mode = afl->fsrv.cs_mode;
    afl->hastemode_fsrv.qemu_mode = afl->fsrv.qemu_mode;
    afl->hastemode_fsrv.frida_mode = afl->fsrv.frida_mode;
    ...
    ACTF("Allocating haste recording map (4GB)");
    /* 4GB memory is not a good choice, may optimize later */
    afl->hastemode_records_map = ck_alloc(MAX_ALLOC);

    OKF("Hastemode is ready");
  }
